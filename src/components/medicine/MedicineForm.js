...
const MedicineForm = (props) => {
  ...
  const {
    ...
    inputBlurHandler: typeBlurHandler,
      ...
      value => medicineTypeList.map(item => item.value).includes(value),
      medicineTypeList[0].value);

  const formIsValid = nameIsValid && unitPriceIsValid && unitsIsValid &&
      typeIsValid && minUnitsIsValid;
          ...
          <Grid container spacing={4}>
            ...
            <Grid item xs={6}>
              <FormControl fullWidth>
                ...
                <OutlinedInput
                    ...
                    label="Unit Price"
                    value={unitPrice}
                    onChange={(event) => unitPriceChangeHandler(
                        event.target.value)}
                    onBlur={unitPriceBlurHandler}
                />
                {unitPriceHasError && <FormHelperText error>Unit price must be >
                  0</FormHelperText>}
              </FormControl>
            </Grid>
            <Grid item xs={6}>
            ...
            <Grid item xs={6}>
              <TextField
                  ...
                  type="number"
                  value={minUnits}
                  onChange={(event) => minUnitsChangeHandler(
                      event.target.value)}
